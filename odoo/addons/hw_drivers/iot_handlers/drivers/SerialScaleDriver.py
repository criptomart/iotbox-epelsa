...
EpelsaProtocol = ScaleProtocol(
    ...
    timeout=1,
    writeTimeout=None,
    measureRegexp=b"[A-Z]\s*(-?[0-9.]+)\r",  # \x02A  0.450\r\x03
    statusRegexp=None,
    commandTerminator=b"",
    commandDelay=0.2,
    measureDelay=0.2,
    ...
    clearCommand=None,
    emptyAnswerValid=False,
    autoResetWeight=True,
)


class EpelsaDriver(ScaleDriver):
    """Driver for the Epelsa/Minerva serial scale."""
    _protocol = EpelsaProtocol
    priority = 1

    def __init__(self, identifier, device):
        super(EpelsaDriver, self).__init__(identifier, device)
        self.device_manufacturer = 'Epelsa'
    ...
    def supported(cls, device):
        ...
        try:
            with serial_connection(device['identifier'], protocol, is_probing=True) as connection:
                ...
                if match:
                    _logger.info("device Epelsa supported")
                    return True
        except serial.serialutil.SerialTimeoutException:
            pass
        except Exception:
    ...
    def _read_weight(self):
        """Asks for a new weight from the scale, checks if it is valid and, if it is, makes it the current valu$"""
